...
- Using an existing private WhatsApp group as the ingestion source in v1.
...
- Integration status chips for WhatsApp capture and Drive sync freshness.
...
- As a user, I want to log UPI expenses by sending a WhatsApp message without opening another app.
...
### 5. UPI Tracking via WhatsApp Direct Message
- The capture surface for v1 is a Meta Cloud API test phone number, not a private WhatsApp group.
- The user and spouse should send direct WhatsApp messages to the app's test number.
- Messages from approved household participants should trigger a workflow that creates or updates transaction candidates.
...
- The system may send ingestion status replies on WhatsApp within the active customer-service window, but WhatsApp is not a required notification channel for v1.
- The implementation should stay within the constraints of Meta test resources and assume only a small approved participant list in v1.
...
- WhatsApp replies are optional ingestion-status acknowledgements, not the primary notification system.
...
1. User or spouse sends a direct message to the app's Meta test number.
2. Meta delivers the inbound message to the webhook workflow.
...
7. If appropriate, the system may send a short status acknowledgement back on WhatsApp within the active reply window.
...
- Statement parser profile configuration.
